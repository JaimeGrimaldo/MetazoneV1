...
import 'package:metazone/pages/meta.dart';
...
class Dashboard extends StatefulWidget {
  final String juego;
  const Dashboard(this.juego, {Key? key}) : super(key: key);
  @override
...
class _DashboardState extends State<Dashboard> with TickerProviderStateMixin {
  ...
  void BestPost() {
    ...
    _listImage = auth.better('rifle');
  }

  void BestPostJaime() {
    print("ENTRO AL DE JAIME");
    final http = Http(baseUrl: 'http://18.217.171.242:9000');
    final AuthenticationRepository auth = AuthenticationRepositoryImpl(
      AuthenticationAPI(http),
    );
    _listImage = auth.better('ar');
  }
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: ((context) => PreDashboardWidget()),
            ),
          );
        },
  ...
  List<Widget> _listImageChildren(data) {
    ...
    for (var i = 0; i < datas.length; i++) {
      print("ESTO ES POS EL TAM ${datas.length}");
      imgs.add(
        SizedBox(
